...
CHANNEL_INVITE_LINK = os.getenv("CHANNEL_INVITE_LINK", "https://t.me/otpservers")  # Required join link
...
# ----------------------------------------------------------------------
# Channel membership check & prompt
# ----------------------------------------------------------------------
...
async def send_join_prompt(target, bot: Bot, user_id: int):
    text = (
...
async def ensure_member(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check membership. If not a member, send join prompt and return False."""
    user_id = update.effective_user.id
    if await is_member(context.bot, user_id):
        return True
    if update.callback_query:
        await send_join_prompt(update.callback_query, context.bot, user_id)
    else:
        await send_join_prompt(update, context.bot, user_id)
    return False

async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send the main menu after successful verification / start."""
    ...
    if update.callback_query:
        await update.callback_query.message.reply_text("Welcome! Choose an option:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))
    else:
...
def main():
    ...
    # Fake Name conversation with fallback
    ...
    logger.info("Bot started (Site1, Site2, Site4, Site6) with channel join verification. Polling...")
